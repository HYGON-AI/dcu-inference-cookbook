...
5. **启动命令**：
   - 必须向用户展示以下固定询问话术，不能只作为内部规则：
     ```text
     请提供完整的 `vllm serve` 或 `sglang serve` 启动命令；没有现成命令可回复“没有”。如果希望参考已有模型命令，可回复“没有，参考 <模型名>”。

     如果提供命令，我会按照项目 `CONTRIBUTING` / 文档贡献要求进行检查和最小必要规范化，不会修改关键必要命令,你可按需修改。

     如果你选择了多个硬件平台，请按硬件分别提供或标注命令；若命令没有区分硬件，我会把同一份命令复制到你选择的所有硬件下面。

     如果回复“没有”，我会按模型和硬件生成模板命令；如果附参考模型名，我会优先参考已有部署文档中该模型的命令，并在最终回复说明参考来源和必要替换。
     ```
   - 若用户提供了完整命令，按照上述话术和“启动命令规范化要求（提供命令时）”处理。
   - 若用户提供了完整命令，且选择多个硬件平台但命令没有逐段明确区分硬件，按上述话术把同一份命令复制到用户选择的所有硬件下面。
   - 若用户未提供命令，根据模型信息、硬件平台和参考模型名（如有）自行查找参考并**生成模板命令**；具体规则见“启动命令模板生成规则（未提供命令时）”。
...
- 模型与端口：若用户命令中的模型路径或模型名称与信息收集阶段用户指定的模型 ID 不一致，必须提示用户将本地路径或非规范模型名称替换为信息收集阶段用户指定的官方模型 ID，并在获得用户确认后才能继续生成文档；同时恢复框架默认端口（vLLM `8000`、SGLang `30000`），除非用户明确要求保留自定义端口。相关替换说明只写在最终回复中，不得写入部署 md 文档正文。
...
1. 若用户额外提供参考模型名，优先在已有部署文档中查找该参考模型的命令；否则在已有部署文档中查找同框架、同版本、架构/量化方式相近的启动命令。
2. 找到参考命令后，可直接作为模板来源生成启动命令；最终回复中说明参考了哪个已有模型/文件，以及做了哪些必要替换或估算。
3. 仍无合适参考时，生成基础模板命令，只使用用户指定模型 ID、按规则估算的 TP/卡数、框架通用必要参数和量化必要参数，并在最终回复说明未套用其他模型的优化参数。
...
- `tensor-parallel-size` / `--tp-size` / `-tp` 必须按下方显存规则重新估算，不能直接沿用参考命令里的卡数：
  ...
  - 根据模型权重和单卡显存计算 TP：`TP = model_weight_GB / gpu_mem_GB`，向上取最近的 2 的幂，并预留 20%~30% 显存余量。
